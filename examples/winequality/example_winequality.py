...
"""
Programa de ejemplo de como usar el paquete Alenux con el dataset de vinos
""" 
import sys
import os
...
nan_value            =   0          # El valor que se usará cuando los datos estén vacios en proyecto
sel_method_test      =   'Hybrid_Score'     # Podemos usar cualquier método definido
sel_method_train     =   'Hybrid_Score'
scope_train          =   2          # La cantidad de candidatos que tendremos en cuenta para aplicar el algoritmo en entrenamiento
scope_test           =   30         # La cantidad de candidatos en el test
stop_train           =   True      # Si paramos en el entramiento cuando llegamos al 100%
...
mapa_algoritmos      =  'map_methods.json'  # El fichero Json que se usará para el mapa de algoritmos
...
# Sacamos el mapa de algoritmos que mejor aplica a este dataset, para ello entrenamos con los datos de entrenamiento2
nv=proyecto.actions.train(dataset=dataset_desc,engine=engine,analytics=analisis_train,row_start=0,rows=0,scope=scope_train,filter_rows=train2,nan_value=nan_value,epochs=epocas,stop=stop_train,mode=mode_train)
    # Ahora hacemos un test con los datos de validación y vemos como es el mapa y lo guardamos en una variable
proyecto.actions.test(dataset=dataset_desc,engine=engine,analytics=analisis_test,row_start=0,rows=0,scope=scope_test,filter_rows=validation,nan_value=nan_value)
map_certainty=analisis_test.get_best_method()
analisis_test.set_methods_map(map_certainty)
    # Comprobamos las mejoras del mapa calculado
proyecto.actions.test(dataset=dataset_desc,engine=engine,analytics=analisis_test,row_start=0,rows=0,scope=scope_test,filter_rows=validation,nan_value=nan_value)
# Convertimos el dataset a neurovectores sin entrenamiento
# nv=proyecto.actions.convert(dataset=vinos_proyecto,engine=engine,filter_rows=conversion)
...
nv=proyecto.actions.train(dataset=dataset_desc,engine=engine,analytics=analisis_train,row_start=0,rows=0,scope=2,filter_rows=validation,nan_value=nan_value,epochs=epocas,stop=stop_train,mode=mode_train)
